@extends('index')
@section('title', 'CategoriesManagemnet')
@section('content')
<div class="container-login">
    @auth
        <div class="row justify-content-center container ">
            ...
            <div class="col-lg-9">
                <div class="card">
                    <center class="card-header">{{ __('Categories') }}</center>
                    <div class="card-body">
                        @if (session('message'))
                        ...
                        <table class="table table-bordered">
                            <h2><a href="{{route('admin.categoriesManagement.createCateForm')}}" class="btn btn-success">Add</a></h2>
                            <thead>
                                <tr>
                                    <th>{{ __('ID') }}</th>
                                    <th>{{ __('Name') }}</th>
                            ...
                            <tbody>
                                @foreach ($categories as $cate)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.categoriesManagement.editCate', $cate) }}" class="btn btn-warning">Edit</a>
                                            <form action="{{route('admin.categoriesManagement.deleteCate',$cate)}}" method="post">
                                                @csrf
                                                @method('DELETE')
